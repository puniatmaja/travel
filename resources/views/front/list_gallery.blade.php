...
@if($list->currentPage() == 1)  
  <title>Gallery - {{$seo->seo_judul}}</title>  
@else
  <title>Gallery - {{$seo->seo_judul}} - Page{{$list->currentPage()}}</title>  
@endif  
@else  
@if($list->currentPage() == 1)  
  <title>Gallery - {{$seo->judul}}</title>  
@else
  <title>Gallery - {{$seo->judul}} - Page{{$list->currentPage()}}</title>  
@endif 
@endif
@if(trim($seo->seo_deskripsi) != '')
  <meta name="description" content="Gallery - <?= substr(strip_tags($seo->seo_deskripsi), 0,300) ?>">
@else
  <meta name="description" content="Gallery - <?= substr(strip_tags($main['profile_website']->deskripsi), 0,300) ?>">
@endif
...
@endif
@if(trim($seo->seo_deskripsi) != '')
  <meta property="og:description"        content="Gallery - <?= substr(strip_tags($seo->seo_deskripsi), 0,300) ?>" />
@else
  <meta property="og:description"        content="Gallery - <?= substr(strip_tags($main['profile_website']->deskripsi), 0,300) ?>" />
@endif
...
@if(trim($seo->seo_deskripsi) != '')
  <meta name="twitter:description" content="Gallery - <?= substr(strip_tags($seo->seo_deskripsi), 0,300) ?>" />
@else
  <meta name="twitter:description" content="Gallery - <?= substr(strip_tags($main['profile_website']->deskripsi), 0,300) ?>" />
@endif
@if(trim($seo->seo_judul) != '')
...
@foreach($list as $row)
      <div class="w3-col l4 m4 w3-margin-bottom ">
        ...
        <div class="w3-display-container">
          <img src="{{url('galeri/'.$row->gambar)}}" alt="{{$row->judul}}" style="width:100%">
        </div>
